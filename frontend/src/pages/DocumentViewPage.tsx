...
const DocumentViewPage = memo(() => {
  ...
  // Tracks current tool as a ref so wheel/mouse handlers don't go stale
  const toolRef = useRef<DrawTool>("select");

  // ─── Loading state ───
  // Phase: 0=idle/auth, 1=fetching doc info, 2=loading PDF, 3=ready
  const [pdfLoadProgress, setPdfLoadProgress] = useState(0);
  const [pdfLoadPhase, setPdfLoadPhase] = useState<
    "idle" | "connecting" | "loading" | "rendering" | "ready"
  >("idle");
  ...
  // Keep toolRef in sync with tool state
  useEffect(() => {
      ...
      if (scrollMode === "continuous") {
        ...
        if (container) {
          ...
          const scrollTop =
            containerPaddingTop +
            innerMarginTop +
            (pageIndex - 1) * (pageH + pageGap);
          container.scrollTo({ top: scrollTop, behavior: "smooth" });
        }
      }
    },
    [scrollMode, numPages, pageDimensions.height, pageDimensions.width, zoom],
  );
  ...
  const handleJumpToMarkup = useCallback(
    (ids: string[]) => {
      ...
      setTimeout(() => {
        ...
        const w = pageDimensions.width * s,
          h = pageDimensions.height * s;
        let markupX = 0,
          markupY = 0,
          markupW = 80,
          markupH = 40;
        if (coords.left !== undefined) {
          markupX = coords.left * w;
          markupY = coords.top * h;
          markupW = (coords.width || 0.1) * w;
          markupH = (coords.height || 0.05) * h;
        } else if (coords.x1 !== undefined) {
          markupX = Math.min(coords.x1, coords.x2) * w;
          markupY = Math.min(coords.y1, coords.y2) * h;
          markupW = Math.abs(coords.x2 - coords.x1) * w;
          markupH = Math.abs(coords.y2 - coords.y1) * h;
        }
        ...
        if (scrollMode === "continuous")
          scrollTop += m.pageNumber * (pageDimensions.height * s + 16);
        container.scrollTo({
  ...
  const handleJumpToSearchMatch = useCallback(
    (idx: number) => {
      ...
      setTimeout(() => {
        ...
        if (Math.abs(displayScale - targetScale) > 0.05)
          handleZoom(targetScale - displayScale);
        let targetX = match.x * targetScale,
          targetY = match.y * targetScale;
        if (scrollMode === "continuous")
          targetY +=
            match.pageIndex * (pageDimensions.height * targetScale + 16);
  ...
  const handleDownloadClean = useCallback(() => {
    ...
    fetch(`/api/documents/${documentId}/proxy`, {
      ...
      .then((blob) => {
        ...
        a.href = url;
        a.download = doc?.name || `document_${documentId}.pdf`;
        a.click();
  ...
  const handlePasteMarkups = useCallback(async () => {
    ...
    if (scrollMode === "continuous") {
      ...
      targetPageIdx = Math.max(
        0,
        Math.min(numPages - 1, Math.floor((rawY - paddingTop) / pageH)),
      );
      mouseX = Math.max(
        0,
        Math.min(1, (rawX - paddingLeft) / (pageDimensions.width * zoom)),
      );
      mouseY = Math.max(
        0,
        Math.min(
          1,
          (rawY - paddingTop - targetPageIdx * pageH) /
      ...
      mouseX = Math.max(
        ...
        Math.min(
          1,
          (rawX - paddingLeft) / (pageDimensions.width * displayScale),
        ),
      );
      mouseY = Math.max(
        0,
        Math.min(
          1,
          (rawY - paddingTop) / (pageDimensions.height * displayScale),
        ),
      );
    }
    ...
    for (const m of markupClipboard) {
      ...
      if (newCoords.left !== undefined) {
        newCoords.left = Math.max(0, Math.min(1, newCoords.left + offsetX));
        newCoords.top = Math.max(0, Math.min(1, newCoords.top + offsetY));
      } else if (newCoords.x1 !== undefined) {
        newCoords.x1 = Math.max(0, Math.min(1, newCoords.x1 + offsetX));
        newCoords.y1 = Math.max(0, Math.min(1, newCoords.y1 + offsetY));
        newCoords.x2 = Math.max(0, Math.min(1, newCoords.x2 + offsetX));
        newCoords.y2 = Math.max(0, Math.min(1, newCoords.y2 + offsetY));
      }
  ...
  useEffect(() => {
    ...
    if (!hasLoadedOnce.current) {
      setPdfLoadPhase("connecting");
      setPdfLoadProgress(10);
    }
    setPdfFile({
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => {
      // Read tool from ref so this handler is never stale
    ...
    const handleMouseUp = () => {
      if (isPanning) {
        isPanning = false;
        container.style.cursor = toolRef.current === "pan" ? "grab" : "auto";
      }
    };
    ...
    // Remove 'tool' from deps — toolRef.current is read inside handlers instead.
    // This prevents event listener re-registration on every tool switch, which was
    // causing panning/zoom instability after changing tools.
  ...
  const onDocumentLoadSuccess = async (pdf: any) => {
    ...
    if (!hasLoadedOnce.current) {
      ...
      setTimeout(() => setPdfLoadPhase("ready"), 400);
    }
  };
  ...
  const handleImportAnnotations = useCallback(async () => {
    ...
    const tid = toast.loading(
      `Importing ${embeddedAnnots.length} annotation${embeddedAnnots.length !== 1 ? "s" : ""}…`,
    );
    try {
  ...
  const showLoadingOverlay =
    ...
    pdfLoadPhase !== "ready" &&
    pdfLoadPhase !== "idle";
  const loadPhaseLabel: Record<string, string> = {
      ...
      {showLoadingOverlay && (
        <Box
          sx={{
            ...
            inset: 0,
            zIndex: 2000,
            bgcolor: isDark ? "rgba(10,10,10,0.92)" : "rgba(240,240,240,0.92)",
          ...
          >
            ...
            <Box
              sx={{
                ...
                "@keyframes pulse": {
                  "0%, 100%": { boxShadow: `0 0 0 0 ${alpha(gold, 0.4)}` },
                  "50%": { boxShadow: `0 0 0 12px ${alpha(gold, 0)}` },
                },
              }}
            >
              <ArticleIcon sx={{ fontSize: 36, color: gold }} />
            </Box>
            <Box sx={{ textAlign: "center" }}>
              <Typography sx={{ fontWeight: 700, fontSize: "1.1rem", mb: 0.5 }}>
                {doc?.name || "Opening document…"}
              </Typography>
              <Typography
                variant="caption"
                ...
                sx={{ fontSize: "0.82rem" }}
              >
                {loadPhaseLabel[pdfLoadPhase] || "Loading…"}
              </Typography>
            </Box>
            {/* Progress bar */}
            <Box sx={{ width: "100%" }}>
              ...
              >
                <Typography
                  ...
                  sx={{ fontSize: "0.72rem" }}
                >
                  {pdfLoadProgress < 100 ? `${pdfLoadProgress}%` : "Complete"}
                </Typography>
                <Typography
                  variant="caption"
                  color="text.secondary"
            ...
            {pdfFile && (
              <Box
                sx={{
                  ...
                  minWidth: pageDimensions.width * displayScale + 64,
              ...
              >
                <Document
                  ...
                  onLoadProgress={({ loaded, total }) => {
                    if (!hasLoadedOnce.current && total > 0) {
                      const pct = Math.round((loaded / total) * 60) + 15; // 15-75% range
                      setPdfLoadPhase("loading");
                      setPdfLoadProgress(Math.min(75, pct));
                    }
                  }}
                  loading={<CircularProgress />}
                >
                    ...
                    sx={{
                      ...
                      height:
                        (scrollMode === "continuous"
                          ? pageDimensions.height * numPages
                          : pageDimensions.height) * zoom,
                      my: 2,
                    }}
                  >
                    {scrollMode === "continuous" ? (
                      ...
                      <PageContainer
                        ...
                        pdfHeight={pageDimensions.height}
                        scale={zoom}
                        markups={markupsByPage[currentPage - 1] || []}
                        tool={tool}
                        activeColor={activeColor}
                        activeStrokeWidth={activeStrokeWidth}
                        activeLineStyle={activeLineStyle}
        ...
        {isSM && (
          ...
          >
            ...
            <Box display="flex" alignItems="center">
              <IconButton
                size="small"
                onClick={() => handleJumpToPage(Math.max(1, currentPage - 1))}
            ...
            <IconButton
              size="small"
              onClick={() =>
                setScrollMode(scrollMode === "page" ? "continuous" : "page")
              }
              sx={{
                p: 0.5,
                color: "text.secondary",
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 0.25 }}>
              ...
              >
                <Typography
                  sx={{ fontSize: "1rem", lineHeight: 1, fontWeight: 400 }}
                >
                  −
                </Typography>
              </IconButton>
            ...
            <Select
              ...
              sx={{
                ...
                color: "text.primary",
                minWidth: 44,
                "& .MuiSelect-select": { p: "0 4px", lineHeight: "20px" },
              }}
              MenuProps={{
                PaperProps: {
                  sx: {
