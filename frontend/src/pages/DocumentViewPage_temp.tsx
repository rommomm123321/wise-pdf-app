...
import CompareDialog, { type CompareConfig } from "../components/pdf/CompareDialog";
...
const DocumentViewPage = memo(() => {
  ...
  const mobileDragRef = useRef<{
    ...
    currentX: number;
    currentY: number;
  }>({ isDragging: false, startPointerX: 0, startPointerY: 0, startPosX: 0, startPosY: 0, currentX: 0, currentY: 0 });
  const [mobileToolbarStyle, setMobileToolbarStyle] = useState<React.CSSProperties>({
    left: '50%',
    bottom: '16px',
    top: 'auto',
  ...
  const handleJumpToMarkup = useCallback(
    (ids: string[]) => {
      ...
      if (tileViewerRef.current) {
        ...
        const doNavigate = () => {
          tileViewerRef.current!.navigateToPagePoint(m.pageNumber, cx, cy, TARGET_ZOOM);
    ...
    [markups, pageDimensions, displayScale, handleJumpToPage, scrollMode, currentPage],
  );
  ...
  const handleJumpToSearchMatch = useCallback(
    (idx: number) => {
      ...
      if (tileViewerRef.current) {
        ...
        const cx = ((match.x || 0) + (match.width || match.w || 0) / 2) * coordScale;
        const cy = ((match.y || 0) + (match.height || match.h || 0) / 2) * coordScale;
    ...
    async (startPoint: { x: number; y: number }) => {
      ...
      const epData = endpoints.map((m: any) => {
        ...
        if (c.points && c.points.length > 0) {
          cx = c.points.reduce((s: number, p: any) => s + p.x, 0) / c.points.length;
          cy = c.points.reduce((s: number, p: any) => s + p.y, 0) / c.points.length;
        } else {
  ...
  const handleMarkupAction = useCallback(
    (action: string, markupId: string) => {
      ...
      handleUpdateProperties(markupId, {
        properties: { ...m.properties, zIndex: newZ },
      });
    },
  ...
  const handleStartCompare = useCallback(async (config: CompareConfig) => {
    ...
    const toastId = toast.loading('Preparing revisions for comparison...');
    try {
      // Prepare BOTH documents in tile server (parallel)
      const [r1, r2] = await Promise.all([
        fetch(`/prepare/${config.oldDocId}?token=${encodeURIComponent(token)}`, { method: 'POST' }),
        fetch(`/prepare/${config.newDocId}?token=${encodeURIComponent(token)}`, { method: 'POST' }),
      ]);
      if (!r1.ok) throw new Error(`prepare old failed: ${r1.status}`);
      if (!r2.ok) throw new Error(`prepare new failed: ${r2.status}`);
      toast.loading('Loading comparison overlay...', { id: toastId });
    } catch (e) {
      toast.error('Failed to prepare revisions. Try again.', { id: toastId, duration: 3000 });
  ...
  const handleExportCompare = useCallback(async () => {
    ...
    try {
      const oldDocLabel = (() => {
        const idx = (doc?.versions || []).findIndex((v: any) => v.id === compareConfig.oldDocId);
      ...
      await downloadComparisonPdf({
        ...
        pageMapping: compareConfig.pageMode === 'custom'
          ? (() => {
              const map: number[] = [];
              for (let i = 0; i < numPages; i++) {
                const pair = compareConfig.customMapping.find(p => p.newPage === i);
        ...
        onProgress: (current, total) => {
          const phases = ['Downloading PDFs...', 'Embedding pages...', 'Building layers...', 'Saving PDF...'];
          toast.loading(phases[current] || `Processing...`, { id: toastId });
        },
      });

      toast.success('Comparison PDF downloaded', { id: toastId, duration: 3000 });
    } catch (e) {
      console.error('Compare export failed:', e);
      toast.error('Failed to export comparison PDF', { id: toastId, duration: 3000 });
    } finally {
      setCompareProcessing(false);
    }
  ...
  const handleSaveCompare = useCallback(async () => {
    ...
    const toastId = toast.loading('Generating comparison PDF...');
    try {
      const { generateComparisonPdf } = await import('../utils/exportComparisonPdf');
      const oldDocLabel = (() => {
        const idx = (doc?.versions || []).findIndex((v: any) => v.id === compareConfig.oldDocId);
        return idx >= 0 ? `Rev${(doc?.versions?.length || 0) - idx}` : 'old';
      })();
      ...
      const bytes = await generateComparisonPdf({
        ...
        onProgress: (cur, total) => {
          const phases = ['Downloading...', 'Embedding pages...', 'Building layers...', 'Finalizing...'];
      ...
      formData.append('file', new Blob([bytes.buffer as ArrayBuffer], { type: 'application/pdf' }), filename);
    ...
    try {
      ...
      await exportPdfWithMarkups({
        ...
        onProgress: (current, total) => {
          ...
          setExportProgress({ current, total, phase: current >= total ? 'Saving...' : `Page ${current}/${total}` });
          toast.loading(
            `Exporting: ${current >= total ? 'Saving PDF...' : `Page ${current} of ${total} (${pct}%)`}`,
            { id: toastId },
          );
        },
      });
      toast.success(`Exported ${doc?.name || 'document'} with markups`, { id: toastId, duration: 3000 });
    } catch (e) {
      console.error("PDF export failed:", e);
      toast.error('Export failed. Please try again.', { id: toastId, duration: 4000 });
    } finally {
  ...
  const handleDownloadClean = useCallback(async () => {
    ...
    try {
      ...
      triggerBlobDownload(blob, doc?.name || `document_${documentId}.pdf`);
      toast.success(`Downloaded ${doc?.name || 'document'}`, { id: toastId, duration: 2000 });
    } catch (e) {
    ...
    reader.onload = async () => {
      ...
      img.onload = async () => {
        ...
        const res = await createMarkup({
          ...
          coordinates: { left: 0.1, top: 0.1, width: targetW, height: Math.min(targetH, 0.8) },
          properties: { imageData: dataUrl, stroke: 'transparent', strokeWidth: 0 },
        });
        if (res?.id) {
  ...
  }, [canMarkup, currentPage, documentId, createMarkup, pushHistory, refetchMarkups]);

  const handlePasteMarkups = useCallback(async (clipboardEvent?: ClipboardEvent) => {
    // Check for image paste from external clipboard first
    if (clipboardEvent?.clipboardData?.files?.length) {
      const file = Array.from(clipboardEvent.clipboardData.files).find(f => f.type.startsWith('image/'));
      if (file) { handlePasteImage(file); return; }
    }
    ...
    if (newIds.length > 0) {
      ...
      refetchMarkups();
      toast.success(`Pasted ${newIds.length} markup${newIds.length > 1 ? 's' : ''} on page ${currentPage}`, { duration: 2000 });
    }
  }, [
    markupClipboard,
    handlePasteImage,
  ...
  const handleMobileDragStart = useCallback((e: React.PointerEvent) => {
    if ((e.target as HTMLElement).closest('[data-nodrag="true"]')) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleMobileDragMove = useCallback((e: React.PointerEvent) => {
    ...
    const newX = Math.max(W / 2 + 8, Math.min(window.innerWidth - W / 2 - 8, mobileDragRef.current.startPosX + dx));
        ...
        try {
          ...
          if (outline && outline.length > 0) {
            ...
            for (const it of flatOutline) {
              ...
              try {
                let destArr = it.dest;
                if (typeof destArr === 'string') destArr = await doc.getDestination(destArr);
              ...
              } catch { /* ignore unresolvable dests */ }
            }
        ...
        if (ocConfig) {
          ...
          if (groups && typeof groups === 'object') {
            ...
            for (const [id, group] of Object.entries(groups) as any) {
              if (group?.name) {
                layers.push({ name: group.name, visible: group.visible !== false });
              }
            }
            setPdfLayers(layers);
          }
        }
      } catch { /* no OCG support or no layers */ }
    } catch (e) {
      console.error(e);
    }
  };
    ...
    >
      {!isSM && <PdfToolbar
        ...
        compareControls={compareConfig ? {
          ...
          showOld: compareShowOld, showNew: compareShowNew,
          oldLabel: (() => { const idx = (doc?.versions || []).findIndex((v: any) => v.id === compareConfig.oldDocId); return idx >= 0 ? `Rev ${(doc?.versions?.length || 0) - idx}` : 'Old'; })(),
          newLabel: (() => { const idx = (doc?.versions || []).findIndex((v: any) => v.id === compareConfig.newDocId); return idx >= 0 ? `Rev ${(doc?.versions?.length || 0) - idx}` : 'New'; })(),
      ...
      >
        ...
        {compareConfig && !isSM && (() => {
          ...
          const layerBtn = (active: boolean, color: string, label: string, toggle: () => void) => (
            <Box onClick={toggle} sx={{
              display:'flex', alignItems:'center', gap:'5px', cursor:'pointer',
              px:1, py:0.4, borderRadius:'8px', flexShrink:0,
              ...
              <Typography sx={{ fontSize:'0.8rem', fontWeight:600, color: active ? color : 'text.disabled' }}>{label}</Typography>
            </Box>
          );
          return (
            <Box sx={{
              ...
              px:1.5, py:'6px', borderRadius:'14px',
              bgcolor: alpha(theme.palette.background.paper, 0.96),
              backdropFilter:'blur(16px)',
              border:`1px solid ${theme.palette.divider}`,
              boxShadow:'0 4px 20px rgba(0,0,0,0.18)',
            }}>
              {layerBtn(compareShowOld, compareConfig.oldColor, cmpOld, () => setCompareShowOld(p => !p))}
              {layerBtn(compareShowNew, compareConfig.newColor, cmpNew, () => setCompareShowNew(p => !p))}
        ...
        {scrollMode === "split" ? (
          ...
          <Box
            sx={{
              ...
              position: "relative",
              overflow: "hidden",
              cursor: (routePanelClickMode || routeMultiClickMode) ? 'crosshair' : undefined,
            }}
            onClick={(routePanelClickMode || routeMultiClickMode) ? (e: React.MouseEvent) => {
              e.stopPropagation();
              const tv = tileViewerRef.current;
              if (!tv) return;
          ...
          >
            {(routePanelClickMode || routeMultiClickMode) && (
              <Box sx={{
                position: 'absolute', top: 8, left: '50%', transform: 'translateX(-50%)', zIndex: 1500,
              ...
              <TileViewer
                ...
                onDocInfo={(info) => {
                  ...
                  if (!pageLabelsFromOutlineRef.current) {
                    const labels = info.pages.map((p, i) => p.label || `${i + 1}`);
                    setPageLabels(labels);
                  }
        ...
        {/* ═══ MOBILE TOOLBAR ═══ */}
        {isSM && (() => {
          const isDrawTool = tool !== 'select' && tool !== 'pan' && tool !== 'textSelect';
          const hasPropsRow = !(['select','pan','textSelect','image'] as DrawTool[]).includes(tool);
          const cmpOldLabel = compareConfig ? (() => { const idx = (doc?.versions || []).findIndex((v: any) => v.id === compareConfig.oldDocId); return idx >= 0 ? `Rev ${(doc?.versions?.length || 0) - idx}` : 'Old'; })() : '';
          const cmpNewLabel = compareConfig ? (() => { const idx = (doc?.versions || []).findIndex((v: any) => v.id === compareConfig.newDocId); return idx >= 0 ? `Rev ${(doc?.versions?.length || 0) - idx}` : 'New'; })() : '';
          const presetColors = ['#f44336','#ff9800','#4caf50','#2196f3','#9c27b0','#000000','#ffffff','#795548','#607d8b'];
          ...
          const TOOL_LABEL: Record<DrawTool, string> = {
            select:'Select', pan:'Pan', textSelect:'Text Sel.',
            pen:'Pen', highlighter:'Highlight', text:'Text Box', image:'Image',
            line:'Line', arrow:'Arrow', measure:'Measure', polyline:'Polyline',
          ...
          const TOOL_ICON: Record<DrawTool, React.ReactNode> = {
            select: <AdsClickIcon />, pan: <PanToolIcon />, textSelect: <AbcIcon />,
            pen: <CreateIcon />, highlighter: <HighlightIcon />, text: <TextFormatIcon />, image: <ImageOutlinedIcon />,
            line: <HorizontalRuleIcon />, arrow: <EastIcon />, measure: <StraightenIcon />, polyline: <PolylineIcon />,
            rect: <RectangleIcon />, circle: <CircleOutlinedIcon />,
            ellipse: <CircleOutlinedIcon sx={{ transform: 'scaleX(1.4) scaleY(0.75)' }} />,
            triangle: <ChangeHistoryIcon />, diamond: <ChangeHistoryIcon sx={{ transform: 'rotate(45deg) scale(0.9)' }} />,
            ...
            callout: (
              <Box sx={{ position:'relative', display:'inline-flex', lineHeight:0 }}>
                <CloudQueueIcon />
                <Box component="span" sx={{ position:'absolute', bottom:-1, right:-5, fontSize:'0.6rem', fontWeight:900, fontFamily:'monospace' }}>+</Box>
              </Box>
            ),
          };
          ...
          const getPlacement = () => {
            const el = mobileToolbarRef.current;
            if (!el) return { ao: { vertical:'top' as const, horizontal:'center' as const }, to: { vertical:'bottom' as const, horizontal:'center' as const } };
            const rect = el.getBoundingClientRect();
            if (rect.top + rect.height / 2 > window.innerHeight * 0.5)
              return { ao: { vertical:'top' as const, horizontal:'center' as const }, to: { vertical:'bottom' as const, horizontal:'center' as const } };
            return { ao: { vertical:'bottom' as const, horizontal:'center' as const }, to: { vertical:'top' as const, horizontal:'center' as const } };
          };
          ...
          const ToolChip = ({ t: tKey }: { t: DrawTool }) => {
            ...
            return (
              <Box
                onClick={() => { setTool(tKey); setMobileToolsOpen(false); }}
                sx={{
                  display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
                  gap:'4px', px:'8px', py:'9px', borderRadius:'12px', cursor:'pointer',
              ...
              >
                {TOOL_ICON[tKey]}
                <Typography sx={{ fontSize:'0.58rem', fontWeight:600, lineHeight:1, textAlign:'center', color:'inherit', whiteSpace:'nowrap' }}>
                  {TOOL_LABEL[tKey]}
                </Typography>
              </Box>
            );
          ...
          const SectionLabel = ({ children }: { children: string }) => (
            <Typography sx={{ fontSize:'0.57rem', fontWeight:800, textTransform:'uppercase', letterSpacing:'0.09em', color:'text.disabled', mb:0.75, px:0.25 }}>
              {children}
            </Typography>
          );
          ...
          return (
            <>
              ...
              <Box
                ...
                sx={{
                  position:'fixed', ...mobileToolbarStyle,
              ...
              >
                ...
                {compareConfig && (
                  <Box display="flex" alignItems="center" sx={{
                    ...
                    <Box onClick={() => setCompareShowOld(p => !p)} sx={{
                      display:'flex', alignItems:'center', gap:'3px', cursor:'pointer',
                      px:0.5, py:0.25, borderRadius:'5px', flexShrink:0,
                      bgcolor: compareShowOld ? alpha(compareConfig.oldColor, 0.15) : 'transparent',
                      border:`1.5px solid ${compareShowOld ? compareConfig.oldColor : theme.palette.divider}`,
                    }}>
                      <Box sx={{ width:7, height:7, borderRadius:1, bgcolor:compareConfig.oldColor }} />
                      <Typography sx={{ fontSize:'0.62rem', fontWeight:600, color: compareShowOld ? compareConfig.oldColor : 'text.disabled' }}>{cmpOldLabel}</Typography>
                    </Box>
                    <Box onClick={() => setCompareShowNew(p => !p)} sx={{
                      display:'flex', alignItems:'center', gap:'3px', cursor:'pointer',
                      px:0.5, py:0.25, borderRadius:'5px', flexShrink:0,
                      bgcolor: compareShowNew ? alpha(compareConfig.newColor, 0.15) : 'transparent',
                      border:`1.5px solid ${compareShowNew ? compareConfig.newColor : theme.palette.divider}`,
                    }}>
                      <Box sx={{ width:7, height:7, borderRadius:1, bgcolor:compareConfig.newColor }} />
                      <Typography sx={{ fontSize:'0.62rem', fontWeight:600, color: compareShowNew ? compareConfig.newColor : 'text.disabled' }}>{cmpNewLabel}</Typography>
                    </Box>
                    <Slider size="small" value={compareConfig.opacity} min={10} max={90} step={5}
                      onChange={(_,v) => setCompareConfig(prev => prev ? {...prev, opacity: v as number} : null)}
                ...
                <Box display="flex" alignItems="center" sx={{
                  ...
                  <Box onPointerDown={handleMobileDragStart}
                    sx={{ display:'flex', alignItems:'center', px:'2px', color:'text.disabled', cursor:'grab', touchAction:'none', '&:active':{cursor:'grabbing'} }}>
                    <DragIndicatorIcon sx={{ fontSize:15 }} />
                  </Box>

                  {/* Sidebar */}
                  <IconButton size="small" onClick={() => setSidebarOpen(!sidebarOpen)} sx={tbBtn(sidebarOpen)}>
                    {sidebarOpen ? <MenuOpenIcon sx={{ fontSize:19 }} /> : <MenuIcon sx={{ fontSize:19 }} />}
                  </IconButton>

                  <Divider orientation="vertical" flexItem sx={{ height:18, mx:'3px', alignSelf:'center', opacity:0.3 }} />

                  {/* Page navigation */}
                  <IconButton size="small" onClick={() => handleJumpToPage(Math.max(1, currentPage - 1))} disabled={currentPage <= 1} sx={tbBtn()}>
                    <KeyboardArrowLeftIcon sx={{ fontSize:18 }} />
                  </IconButton>
                  <Box sx={{ display:'flex', alignItems:'center', gap:'1px', px:'4px', py:'2px', bgcolor:alpha(gold, 0.07), borderRadius:'7px', mx:'1px' }}>
                    <InputBase value={currentPage}
                      onChange={(e) => { const v = parseInt(e.target.value); if (!isNaN(v)) handleJumpToPage(v); }}
                      sx={{ width:20, '& input':{ textAlign:'center', p:0, fontSize:'0.72rem', fontWeight:700 } }} />
                    <Typography sx={{ fontSize:'0.62rem', opacity:0.45 }}>/</Typography>
                    <Typography sx={{ fontSize:'0.72rem', fontWeight:600 }}>{numPages}</Typography>
                  </Box>
                  <IconButton size="small" onClick={() => handleJumpToPage(Math.min(numPages, currentPage + 1))} disabled={currentPage >= numPages} sx={tbBtn()}>
                    <KeyboardArrowRightIcon sx={{ fontSize:18 }} />
                  </IconButton>

                  {/* Scroll mode toggle */}
                  <IconButton size="small"
                    onClick={() => setScrollMode(scrollMode === 'continuous' ? 'page' : 'continuous')}
                    title={scrollMode === 'continuous' ? 'Single page' : 'Continuous scroll'}
                    sx={tbBtn(false)}>
                  ...
                  {canMarkup && (
                    <>
                      <Divider orientation="vertical" flexItem sx={{ height:18, mx:'3px', alignSelf:'center', opacity:0.3 }} />
                      <IconButton size="small" onClick={() => setMobileToolsOpen(true)} sx={{
                        p:'6px', borderRadius:'9px',
                        color: isDrawTool ? gold : 'text.secondary',
                        bgcolor: isDrawTool ? alpha(gold, 0.12) : 'transparent',
                        '& svg':{ fontSize:19 },
                        '&:hover':{ bgcolor:alpha(gold,0.12), color:gold },
                      }}>
                        {TOOL_ICON[tool]}
                  ...
                  {/* Undo / Redo */}
                  <IconButton size="small" onClick={handleUndo} disabled={!canUndo} sx={tbBtn()}>
                    <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M12.5 8c-2.65 0-5.05.99-6.9 2.6L2 7v9h9l-3.62-3.62c1.39-1.16 3.16-1.88 5.12-1.88 3.54 0 6.55 2.31 7.6 5.5l2.37-.78C21.08 11.03 17.15 8 12.5 8z"/></svg>
                  </IconButton>
                  <IconButton size="small" onClick={handleRedo} disabled={!canRedo} sx={tbBtn()}>
                    <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M18.4 10.6C16.55 8.99 14.15 8 11.5 8c-4.65 0-8.58 3.03-9.96 7.22L3.9 16c1.05-3.19 4.05-5.5 7.6-5.5 1.95 0 3.73.72 5.12 1.88L13 16h9V7l-3.6 3.6z"/></svg>
                  </IconButton>
                  ...
                  {/* Doc scale */}
                  <Select size="small" value={docScale} onChange={(e) => handleDocScaleChange(e.target.value as string)}
                    variant="standard" disableUnderline IconComponent={() => null}
                    sx={{ fontSize:'0.63rem', fontWeight:700, color:'text.secondary', minWidth:36, '& .MuiSelect-select':{ p:'0 2px' } }}
                    ...
                    {STANDARD_SCALES.map((g: any, idx: number) =>
                      g.items
                        ? [
                          <ListSubheader key={`h-${idx}`} sx={{ lineHeight:'28px', fontSize:'0.58rem', fontWeight:800, textTransform:'uppercase', color:gold, bgcolor:'background.paper' }}>{g.group}</ListSubheader>,
                          ...g.items.map((item: any) => (
                            <MenuItem key={item.value} value={item.value} sx={{ fontSize:'0.75rem' }}>{item.label}</MenuItem>
                          ))
                        ]
                        : <MenuItem key={g.value} value={g.value} sx={{ fontSize:'0.75rem', fontWeight:700 }}>{g.label}</MenuItem>
                    )}
                  </Select>
                  ...
                  {doc?.versions && doc.versions.length > 0 && (
                    <>
                      <Divider orientation="vertical" flexItem sx={{ height:18, mx:'3px', alignSelf:'center', opacity:0.3 }} />
                      <Select size="small" value={documentId} variant="standard" disableUnderline IconComponent={() => null}
                        onChange={(e) => { window.location.href = `/projects/${projectId}/documents/${e.target.value}`; }}
                        sx={{ fontSize:'0.63rem', fontWeight:700, minWidth:52, '& .MuiSelect-select':{ p:'0 2px' } }}
                        MenuProps={{ PaperProps: { sx:{ bgcolor:'background.paper', borderRadius:'12px', boxShadow:'0 4px 20px rgba(0,0,0,0.2)' } } }}>
                ...
                {hasPropsRow && <Box sx={{
                  ...
                  <Box component="label" sx={{ position:'relative', flexShrink:0, cursor:'pointer', lineHeight:0 }}>
                    <Box sx={{ width:20, height:20, borderRadius:'50%', bgcolor:activeColor,
                      outline:`2px solid ${alpha(theme.palette.divider,0.5)}`, outlineOffset:'1.5px',
                      boxShadow: activeColor==='#ffffff'?`inset 0 0 0 1px ${alpha('#000',0.15)}`:'none' }} />
                  ...
                  <Box sx={{ display:'flex', alignItems:'center', gap:'5px', flex:1, minWidth:0, px:'2px' }}>
                    <Slider size="small" min={1} max={20} step={1}
                      ...
                      sx={{
                        color: gold, height:3, p:'8px 0',
                        '& .MuiSlider-thumb':{ width:12, height:12, transition:'none', '&:hover':{ boxShadow:`0 0 0 6px ${alpha(gold,0.16)}` } },
                        '& .MuiSlider-rail':{ opacity:0.22 },
                        '& .MuiSlider-track':{ border:'none' },
                      }}
                    />
                    <Typography sx={{ fontSize:'0.65rem', fontWeight:700, minWidth:16, textAlign:'right', color:'text.secondary', flexShrink:0, lineHeight:1 }}>
                      {activeStrokeWidth}
                    </Typography>
                  </Box>
                  ...
                  {/* Line style dropdown */}
                  <Select size="small" value={activeLineStyle} onChange={(e)=>setActiveLineStyle(e.target.value as any)}
                    variant="standard" disableUnderline
                    sx={{ flexShrink:0,
                      '& .MuiSelect-select':{ p:'0 18px 0 2px', display:'flex', alignItems:'center', minHeight:'unset !important' },
                      '& .MuiSelect-icon':{ right:0, color:'text.disabled', fontSize:'16px' },
                    }}
                    renderValue={(val) => <LinePreview style={val as any} previewWidth={42} forceColor={theme.palette.text.secondary} />}
                    MenuProps={{ PaperProps:{ sx:{ bgcolor:'background.paper', borderRadius:'12px', boxShadow:'0 8px 32px rgba(0,0,0,0.2)', minWidth:90 } } }}>
                    {LINE_STYLES.map(ls => (
                      <MenuItem key={ls.key} value={ls.key} sx={{ py:'6px' }}>
                        <LinePreview style={ls.key} previewWidth={60} forceColor={theme.palette.text.primary} />
                      </MenuItem>
                    ))}
                  </Select>
                </Box>}
              ...
              {/* ═══ TOOLS BOTTOM SHEET ═══ */}
              <Box sx={{ display:mobileToolsOpen?'block':'none', position:'fixed', inset:0, bgcolor:'rgba(0,0,0,0.45)', zIndex:1400, backdropFilter:'blur(2px)' }}
                onClick={() => setMobileToolsOpen(false)} />
              <Box sx={{
                position:'fixed', left:0, right:0, bottom:0, zIndex:1401,
                bgcolor:'background.paper', borderRadius:'20px 20px 0 0',
                boxShadow:'0 -8px 48px rgba(0,0,0,0.25)',
                transform: mobileToolsOpen ? 'translateY(0)' : 'translateY(110%)',
                ...
                <Box sx={{ display:'flex', alignItems:'center', justifyContent:'space-between', px:2, pb:1, flexShrink:0 }}>
                  <Typography sx={{ fontSize:'0.95rem', fontWeight:700 }}>Markup Tools</Typography>
                  <IconButton size="small" onClick={() => setMobileToolsOpen(false)} sx={{ color:'text.secondary' }}>
                ...
                <Box sx={{ overflowY:'auto', px:2, pb:3 }}>
                  ...
                  <Box display="flex" gap={0.75} mb={1.5}>
                    <ToolChip t="select" /><ToolChip t="pan" /><ToolChip t="textSelect" />
                  </Box>
                  <SectionLabel>Drawing</SectionLabel>
                  <Box display="flex" gap={0.75} mb={1}>
                    <ToolChip t="pen" /><ToolChip t="highlighter" /><ToolChip t="text" /><ToolChip t="image" />
                  </Box>
                  <SectionLabel>Lines</SectionLabel>
                  <Box display="flex" gap={0.75} mb={1}>
                    <ToolChip t="line" /><ToolChip t="arrow" /><ToolChip t="measure" /><ToolChip t="polyline" />
                  </Box>
                  <SectionLabel>Shapes</SectionLabel>
                  <Box display="flex" gap={0.75} mb={1} flexWrap="wrap">
                    <ToolChip t="rect" /><ToolChip t="circle" /><ToolChip t="ellipse" /><ToolChip t="triangle" />
              ...
              {/* ═══ DOWNLOAD POPUP ═══ */}
              <Popover open={mobileDownloadOpen} anchorEl={mobileToolbarRef.current} onClose={() => setMobileDownloadOpen(false)}
                anchorOrigin={pl.ao} transformOrigin={pl.to}
                slotProps={{ paper:{ sx:{ bgcolor:'background.paper', borderRadius:'16px', boxShadow:'0 12px 48px rgba(0,0,0,0.22)', minWidth:230, overflow:'hidden', mt: pl.ao.vertical==='top'?'-8px':'8px' } } }}>
                <Box sx={{ p:1.5 }}>
                  <Typography sx={{ px:1, pb:0.75, color:'text.disabled', fontWeight:800, textTransform:'uppercase', fontSize:'0.57rem', letterSpacing:'0.08em', display:'block' }}>
                    Download
                  </Typography>
                  <MenuItem onClick={() => { handleDownloadClean(); setMobileDownloadOpen(false); }}
                    sx={{ borderRadius:'10px', gap:1.5, py:1.25, '&:hover':{ bgcolor:alpha(gold,0.08) } }}>
                    <DownloadIcon sx={{ fontSize:20, color:'text.secondary' }} />
                    <Box>
                      <Typography sx={{ fontSize:'0.85rem', fontWeight:600 }}>Without markups</Typography>
                      <Typography sx={{ fontSize:'0.7rem', color:'text.secondary' }}>Clean PDF file</Typography>
                    </Box>
                  </MenuItem>
                  <MenuItem onClick={() => { handleExportPdf(); setMobileDownloadOpen(false); }}
                    sx={{ borderRadius:'10px', gap:1.5, py:1.25, '&:hover':{ bgcolor:alpha(gold,0.08) } }}>
                    <LayersIcon sx={{ fontSize:20, color:gold }} />
                    <Box>
                      <Typography sx={{ fontSize:'0.85rem', fontWeight:600 }}>With markups</Typography>
                      <Typography sx={{ fontSize:'0.7rem', color:'text.secondary' }}>PDF with annotations</Typography>
                    </Box>
                  </MenuItem>
                </Box>
              </Popover>
      ...
      {compareProcessing && (
        <Box sx={{
          ...
          <Box sx={{
            ...
            <Typography sx={{ fontSize: '0.78rem', color: 'text.secondary', textAlign: 'center', maxWidth: 280 }}>
              Please wait while the overlay is being generated. This may take a moment for large documents.
            </Typography>
          </Box>
        </Box>
      )}
